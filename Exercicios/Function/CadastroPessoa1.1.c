...
//protocolo quando entrada é inválida
void invalido(){
...
//retorna a comparação de acordo com o tipo
int tipo_ord(Pessoa temp, Pessoa pessoaj, int tipo, int ordem){
	switch(tipo){
		case 1: return inverte(temp.id < pessoaj.id, ordem);
			break;
		case 2: return inverte(strcmp(temp.nome, pessoaj.nome) < 0, ordem);
			break;
		case 3: return inverte(temp.idade < pessoaj.idade, ordem);
			break;
		case 4: return inverte(temp.altura < pessoaj.altura, ordem);
			break;
		}
		return 0;
}
...
void ordena(Pessoa* pessoas, int cont){
	...
	for(int i = 1; i < cont; i++){
		...
		int j = i-1;
		while(j >= 0 && tipo_ord(temp, pessoas[j], tipo, ordem)){
